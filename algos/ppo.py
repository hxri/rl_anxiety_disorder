...
class PPOAlgo(BaseAlgo):
    ...
    def __init__(self, envs, acmodel, device=None, num_frames_per_proc=None, discount=0.99, lr=0.001, gae_lambda=0.95,
                 ...
                 reshape_reward=None, appraisal=False):
        ...
        if(self.use_appraisal == True):
            print("\nUsing appraisal induced loss")

        # Appraisal targets
        text_file = open('targets.txt', "r")
        lines = text_file.read().split(',')
        arr = []
        for l in lines:
            arr.append(float(l))
        self.appraisal_targets = torch.tensor(arr)
        print("\nAppraisal Targets: {}\n" .format(arr))
            
        # if len(arr) != 0:
        #     self.appraisal_targets = torch.tensor(arr)
        # else:
        #     self.appraisal_targets = torch.tensor([0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5])
    ...
    def update_parameters(self, exps):
        ...
        for epoch in range(self.epochs):
            ...
            for b, inds in enumerate(self._get_batches_starting_indexes()):
                ...
                appraisal = torch.zeros((len(inds), 7))
                dist = None
                ...
                for i in range(self.recurrence):
                    ...
                    # if (self.use_appraisal == True):
                    #     loss = policy_loss - self.entropy_coef * entropy + self.value_loss_coef * value_loss + self.appraisal_coef * appraisal_loss
                    # else:
