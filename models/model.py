import torch
import torch.nn as nn
import torch.nn.functional as F
from torch.distributions.categorical import Categorical
import numpy as np

from .base import RecurrentACModel
from algos.appraisal import motivational_relevance, novelty, certainity, coping_potential, anticipation, goal_congruence


# Function from https://github.com/ikostrikov/pytorch-a2c-ppo-acktr/blob/master/model.py
def init_params(m):
    classname = m.__class__.__name__
    if classname.find("Linear") != -1:
        m.weight.data.normal_(0, 1)
        m.weight.data *= 1 / torch.sqrt(m.weight.data.pow(2).sum(1, keepdim=True))
        if m.bias is not None:
            m.bias.data.fill_(0)


class ACModel(nn.Module, RecurrentACModel):
    def __init__(self, obs_space, action_space, use_text=False, use_appraisal=False):
        super().__init__()

        # Decide which components are enabled
        self.use_text = use_text
        self.use_appraisal = use_appraisal

        # Define image embedding
        self.image_conv = nn.Sequential(
            nn.Conv2d(3, 16, (2, 2)),
            nn.ReLU(),
            nn.MaxPool2d((2, 2)),
            nn.Conv2d(16, 32, (2, 2)),
            nn.ReLU(),
            nn.Conv2d(32, 64, (2, 2)),
            nn.ReLU()
        )
        n = obs_space["image"][0]
        m = obs_space["image"][1]
        self.image_embedding_size = ((n-1)//2-2)*((m-1)//2-2)*64
        print(f'Image embedding size: {self.image_embedding_size}')
        print(f'Semi memory size: {self.semi_memory_size}')

        self.memory_rnn = nn.LSTMCell(self.image_embedding_size, self.semi_memory_size)

        # Define text embedding
        if self.use_text:
            self.word_embedding_size = 32
            self.word_embedding = nn.Embedding(obs_space["text"], self.word_embedding_size)
            self.text_embedding_size = 128
            self.text_rnn = nn.GRU(self.word_embedding_size, self.text_embedding_size, batch_first=True)

        # Resize image embedding
        self.embedding_size = self.semi_memory_size
        if self.use_text:
            self.embedding_size += self.text_embedding_size

        # Define actor's model
        self.actor = nn.Sequential(
            nn.Linear(self.embedding_size, 64),
            nn.Tanh(),
            nn.Linear(64, action_space.n)
        )

        # Define critic's model
        self.critic = nn.Sequential(
            nn.Linear(self.embedding_size, 64),
            nn.Tanh(),
            nn.Linear(64, 1)
        )

        # Initialize parameters correctly
        self.apply(init_params)

    @property
    def memory_size(self):
        return 2*self.semi_memory_size

    @property
    def semi_memory_size(self):
        return self.image_embedding_size# + 3

    def forward(self, obs, memory, dist, appraisal, accountable):
        # Reshape the environment frame and pass it through
        # the convolutional layers to produce a state feature
        x = obs.image.transpose(1, 3).transpose(2, 3)
        x = self.image_conv(x)
        x = x.reshape(x.shape[0], -1)

        # If dist is None, then this is the first step of the episode (t=0).
        if dist is None:
            novelty_values = torch.zeros((x.shape[0], 1))
            certainity_values = torch.zeros((x.shape[0], 1))
            coping_values = torch.zeros((x.shape[0], 1))
            anticipation_values = torch.zeros((x.shape[0], 1))
            goal_congruence_values = torch.zeros((x.shape[0], 1))
        else:
            novelty_values = novelty(dist.logits).unsqueeze(1)
            certainity_values = certainity(dist.logits).unsqueeze(1)
            coping_values = coping_potential(dist.logits).unsqueeze(1)
            anticipation_values = anticipation(dist.logits).unsqueeze(1)
            goal_congruence_values = goal_congruence(dist.logits).unsqueeze(1)

        # print((anticipation(obs.image[..., 0], anticipation_values).unsqueeze(1)).shape)
        appraisal = torch.hstack((
            motivational_relevance(obs.image[..., 0]).unsqueeze(1),
            novelty_values,
            accountable,
            certainity_values,
            coping_values,
            anticipation_values,
            # goal_congruence(obs.image[..., 0], goal_congruence_values).unsqueeze(1)
            goal_congruence_values
        ))

        # print(appraisal)

        # Update the memory with the hidden state, the cell state,
        # and the appraisal. The appraisal is concatenated into the
        # cell state, since appraisal contains episodic information.
        h = memory[:, :self.semi_memory_size]
        # if self.use_appraisal:
        #     c = torch.hstack((memory[:, (self.semi_memory_size + 7):], appraisal)) 
        # else:
        #     c = memory[:, self.semi_memory_size:] # Turn this on for monitoring only mode.
        c = torch.hstack((memory[:, (self.semi_memory_size + 7):], appraisal)) 

        hidden = self.memory_rnn(x, (h, c))
        embedding = hidden[0]
        memory = torch.cat(hidden, dim=1)

        if self.use_text:
            embed_text = self._get_embed_text(obs.text)
            embedding = torch.cat((embedding, embed_text), dim=1)

        x = self.actor(embedding)
        dist = Categorical(logits=F.log_softmax(x, dim=1))

        x = self.critic(embedding)
        value = x.squeeze(1)

        return dist, value, memory, embedding, appraisal 

    def _get_embed_text(self, text):
        _, hidden = self.text_rnn(self.word_embedding(text))
        return hidden[-1]